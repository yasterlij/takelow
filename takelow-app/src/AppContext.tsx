...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const login = useCallback((phone: string, pin: string): boolean => {
    ...
    if (!found || pin !== (found.role === 'admin' ? '1234' : '0000')) return false
    setUser(found)
    setView('home')
    return true
  }, [])
  ...
  const refreshAuctions = useCallback(async () => {
    ...
    try {
      ...
      const mapped: Auction[] = res.data.map((apiAuction) => ({
        ...
        status: apiAuction.status === 'ACTIVE' ? 'live' as const : 'closed' as const,
  ...
  const updateAuction = useCallback(async (id: string, data: Partial<Pick<Auction, "name" | "category" | "marketPrice" | "description" | "highlights">> & { startTime?: string; endTime?: string }) => {
    try {
      const auctionsRes = await api.listAuctions()
      const auction = auctionsRes.data.find(a => a.id === id)
  ...
  const value = useMemo(
    () => ({
      ...
      go, selectAuction, payFee, submitBid, payWinning, setWinner, reset,
