...
import { AppBar, PhoneStatusBar, Badge } from "../ui"
import { useCountdown } from "../countdown"
import { auctions, CURRENCY, formatETB, formatCountdown, type Auction } from "@/lib/auctions"
  ...
  return (
    <span
      className={
        ...
        (endingSoon ? "bg-primary/15 text-primary" : "bg-navy/10 text-navy")
      }
    >
...
export function AuctionsScreen() {
  const { go, selectAuction, myBids } = useApp()
  ...
  return (
    <div className="flex flex-1 flex-col lg:overflow-y-auto">
      <div className="lg:rounded-t-[2rem] lg:overflow-hidden">
          ...
          {categories.map((c) => {
            ...
            return (
              <button
                ...
                className={
                  "flex-shrink-0 rounded-full border px-3.5 py-1.5 text-xs font-semibold transition-colors " +
                  (active
