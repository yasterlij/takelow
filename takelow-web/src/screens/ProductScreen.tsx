...
import { CURRENCY, formatCountdown, formatCurrency, getSpecEntries } from "../mockDataV0"
    ...
    >
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          {auction.specSummary && <p className="mt-1 text-sm font-semibold text-neutral-600">{auction.specSummary}</p>}
        </div>
      </div>
            ...
            <div className="space-y-5">
              ...
              <div className="flex items-stretch gap-2">
                <div className="min-w-0 flex-1 rounded-[1.25rem] bg-gradient-to-r from-awash-blue via-awash-blue-dark to-awash-gold-dark px-2.5 py-2.5 text-white shadow-[0_18px_40px_rgba(0,43,92,0.16)]">
                  ...
                  <p className={`mt-1 truncate font-display text-[22px] font-extrabold tabular-nums ${isEnding ? "text-amber-300" : "text-white"}`}>{countdown.d !== "00" ? `${parseInt(countdown.d)}d ` : ""}{countdown.h}:{countdown.m}:{countdown.s}</p>
                </div>
              ...
              <section className="rounded-[1.75rem] bg-gradient-to-r from-awash-blue via-awash-blue-dark to-awash-gold-dark p-4 text-white shadow-[0_18px_40px_rgba(0,43,92,0.16)]">
                ...
                <div className="mt-3">
                  ...
                  {showPaymentMethods && (
                    <div className="mt-2 rounded-[1.25rem] border border-border/60 bg-white/80 p-2">
                      <button
                        type="button"
                        onClick={() => { setSelectedPaymentMethod("AWASH"); setShowPaymentMethods(false) }}
                      ...
                      <button
                        type="button"
                        onClick={() => { setSelectedPaymentMethod("SIKINAPAY"); setShowPaymentMethods(false) }}
                ...
                <div className={`mt-4 rounded-[1.5rem] border p-4 ${selectedPaymentMethod === "AWASH" ? "border-white/12 bg-white/10" : "border-white/12 bg-white/10"}`}>
                  ...
                  <div className="mt-4 flex items-center justify-between text-xs font-semibold">
                    ...
                    <span className={selectedPaymentMethod === "AWASH" ? (walletBalance < auction.bidFee ? "text-red-200" : "text-white") : "text-white/72"}>
                ...
                <button
                  ...
                  disabled={!hasValidBid || checkingPin || (selectedPaymentMethod === "AWASH" && (loadingMethod === "SIKINAPAY" || walletBalance < auction.bidFee)) || (selectedPaymentMethod === "SIKINAPAY" && loadingMethod === "AWASH")}
                ...
                {walletBalance < auction.bidFee && (
                  <p className="mt-3 rounded-xl border border-destructive/20 bg-destructive/10 px-4 py-3 text-xs font-semibold text-destructive">Awash Wallet balance is below the bid amount. Use SikinaPay or top up your wallet.</p>
                )}
                {authError && (
                  <p className="mt-3 rounded-xl border border-destructive/20 bg-destructive/10 px-4 py-3 text-xs font-semibold text-destructive">{authError}</p>
                )}
              </section>
      ...
      {showPinModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-[1.75rem] border border-border/60 bg-white p-6 shadow-[0_30px_80px_rgba(0,0,0,0.18)]">
            <h3 className="font-display text-xl font-extrabold text-foreground">{needsPinSetup ? "Set Wallet PIN" : "Confirm Wallet PIN"}</h3>
            ...
            {needsPinSetup ? (
              <div className="mt-5 space-y-3">
                <input
                  value={setupPin}
                  onChange={(e) => { setSetupPin(e.target.value.replace(/\D/g, "").slice(0, 6)); setSetupError(null) }}
                ...
                <input
                  value={setupConfirm}
                  onChange={(e) => { setSetupConfirm(e.target.value.replace(/\D/g, "").slice(0, 6)); setSetupError(null) }}
                  placeholder="Confirm PIN"
                  inputMode="numeric"
                  type="password"
                  className="input-full"
                />
                {setupError ? <p className="text-xs font-semibold text-destructive">{setupError}</p> : null}
                <div className="flex gap-3">
                  <button onClick={() => setShowPinModal(false)} className="btn-secondary flex-1">Cancel</button>
                  <button onClick={handleSetupPin} disabled={setupLoading} className="btn-primary flex-1">
              ...
              <div className="mt-5 space-y-3">
                <input
                  ...
                  className="input-full"
                />
                {pinLocked ? <p className="text-xs font-semibold text-destructive">Wallet PIN locked. {lockCountdown || "Try again later."}</p> : null}
                {!pinLocked && attemptsRemaining != null ? <p className="text-xs font-medium text-neutral-500">Attempts remaining: {attemptsRemaining}</p> : null}
                {pinError ? <p className="text-xs font-semibold text-destructive">{pinError}</p> : null}
                <div className="flex gap-3">
                  <button onClick={() => setShowPinModal(false)} className="btn-secondary flex-1">Cancel</button>
                  <button onClick={handleVerifyPin} disabled={pinLoading || pinLocked} className="btn-primary flex-1">
                    {pinLoading ? <Loader2 className="size-4 animate-spin" /> : "Pay with Wallet"}
                  </button>
                </div>
              </div>
            )}
