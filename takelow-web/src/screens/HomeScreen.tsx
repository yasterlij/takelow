...
import { Gavel, Wallet, ArrowRight, Eye, EyeOff, Trophy, Timer, Sparkles, ChevronLeft, ChevronRight, Zap, User } from "lucide-react"
import { useApp } from "../AppContext"
import { AwashMark, Badge, CTAButton } from "../components/AuctionUI"
...
function WinnerShowcaseSlide({ auction, index }: { auction: any; index: number }) {
  ...
  const winnerName = winnerInfo?.name || (winnerInfo?.user_id ? `User ${winnerInfo.user_id.slice(0, 8)}` : `Winner #${index + 1}`)
  const bidAmount = auction.winning_bid_amount ?? winnerInfo?.amount ?? 0
  const maskPhone = (p: string | null) => p ? p.slice(0, 4) + '****' + p.slice(-2) : null
  return (
    <div className="group relative flex w-[280px] flex-shrink-0 snap-start flex-col overflow-hidden rounded-2xl border border-border/50 bg-white/80 backdrop-blur-sm shadow-md transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_12px_40px_rgba(200,166,66,0.15)] hover:border-primary/30">
      <div className="relative h-36 w-full overflow-hidden bg-neutral-100">
        ...
        <div className="absolute bottom-2 left-3">
          <span className="inline-flex items-center gap-1 rounded-full bg-gradient-to-r from-awash-gold/80 to-awash-gold-light/80 backdrop-blur-md px-2.5 py-1 text-[10px] font-bold text-awash-blue border border-primary/20">
            <Trophy className="size-3" /> #{index + 1} Winner
          </span>
        </div>
      </div>
      <div className="flex flex-col gap-2 p-4 pt-3">
        <div className="flex items-center justify-between">
          <div className="min-w-0 flex-1">
            <p className="truncate font-display text-base font-extrabold text-foreground">{winnerName}</p>
            {winnerInfo?.phone && <p className="text-[11px] font-medium text-neutral-400">{maskPhone(winnerInfo.phone)}</p>}
          </div>
          <span className="shrink-0 rounded-full bg-gradient-to-r from-emerald-50 to-emerald-100/80 px-2.5 py-0.5 text-[10px] font-bold text-emerald-700 border border-emerald-200/50">
            {CURRENCY} {formatETB(bidAmount)}
          </span>
        </div>
        <p className="text-xs font-medium text-neutral-500">{auction.name}</p>
        <div className="mt-1 inline-flex items-center gap-1.5 rounded-lg bg-primary/5 backdrop-blur-sm border border-primary/10 px-3 py-1.5">
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 pb-8 stagger-enter">
      ...
      <section>
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="flex size-11 items-center justify-center rounded-xl bg-primary/10 backdrop-blur-sm border border-primary/20 text-awash-gold"><Trophy className="size-5" /></span>
            <div>
              <h2 className="font-display text-xl font-extrabold tracking-tight text-gradient-gold">Winners!</h2>
              <p className="text-sm font-medium text-neutral-500">Celebrate our recent auction winners</p>
            </div>
          </div>
          <button onClick={() => go("closed-auctions")} className="inline-flex items-center gap-1.5 rounded-full bg-awash-blue px-4 py-2 text-sm font-bold text-white transition-all hover:bg-awash-blue-light active:scale-[0.97] shadow-lg shadow-awash-blue/20">
            View All <ArrowRight className="size-4" />
          </button>
        </div>
        ...
        <button onClick={() => go("closed-auctions")} className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-awash-blue/5 backdrop-blur-sm border border-awash-blue/20 px-5 py-3 text-sm font-bold text-awash-blue transition-all hover:bg-awash-blue/10 active:scale-[0.97]">
          <Trophy className="size-4" /> View All Closed Auctions <ArrowRight className="size-4" />
        </button>
      </section>
      )}
